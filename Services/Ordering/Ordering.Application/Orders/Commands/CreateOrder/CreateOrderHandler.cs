    ...
    : ICommandHandler<CreateOrderCommand, CreateOrderResult>
{
    public async Task<CreateOrderResult> Handle(CreateOrderCommand command, CancellationToken cancellationToken)
    {
        ...
        var customer = Customer.Create(CustomerId.Of(command.Order.CustomerId), command.Order.OrderName, "email");
        var products = command.Order.OrderItems.Select(x =>
            Product.Create(ProductId.Of(x.ProductId), "test", x.Price));
